# hangman demo

#https://appbrewery.github.io/python-day7-demo/

# by using for/while , string , lists, range , modules,if/else 

# improving the user-experience

# set the lives for the guess , declaring a variable with some value , upon completing the guess we need to look for new


import random

from words_list import list

# we can keep multiple in single import like from <module> import <part1>,<part2>
# and also you can print(<module>.<partyouwant>)

from hangman_art import stages

from hangman_logo import hangman

print(hangman)

random_word = random.choice(list)

print(random_word)


#placeholder
placeholder = ""
for position in range(len(random_word)):
    placeholder += "_"
print(placeholder)


lives = 6
game_over = False
correct_letters =[]
while not game_over :
    print(f"*******************{lives}/6 Lives left***************")
    user_input = input("Guess a  letter :").lower()

    display = ""
    for letter in random_word:
        if letter == user_input:
            display += letter
            correct_letters.append(letter) #or user_input is also fine since they both are same 
        elif letter in correct_letters:
            display += letter
        else:
            display += "_"
    
    if user_input not in letter:
        lives -= 1
        print(f"you guessed the letter {user_input} thats not in the word , you lose life")
        if lives == 0:
            game_over = True
            print(f"**********It was this {random_word} , you lose*********** ")
        
    print(display)
    if "_" not in display:
        game_over = True
        print("you win.")
    
    print(stages[lives])

